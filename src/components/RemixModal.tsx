...
export default function RemixModal({ block, tripContext, onClose, onSwap }: RemixModalProps) {
  ...
  const searchBar = (
    <div className="mt-3 flex gap-1.5 px-6">
      ...
      <div className="w-full min-w-0 px-6 pb-6 space-y-3">
        ...
        {!loading && !searching && displayAlternatives.map((alt) => (
          <motion.button
            ...
            className="w-full rounded-2xl border border-border/50 bg-white/60 backdrop-blur-sm p-4 text-left hover:border-primary/40 active:border-primary/40 hover:shadow-lg transition-all group"
          >
    ...
    <Dialog open={!!block} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="glass-card-readable max-w-lg w-[95vw] p-0 overflow-hidden rounded-3xl border-white/20 shadow-2xl max-h-[85vh] flex flex-col [&>button]:z-10">
        <div className="px-6 pt-6 pb-3">
          ...
          <div className="-mx-6">{searchBar}</div>
        </div>
        {bodyContent}
      </DialogContent>
    </Dialog>
